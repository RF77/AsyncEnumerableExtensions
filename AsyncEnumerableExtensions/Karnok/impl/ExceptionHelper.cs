...
namespace AsyncEnumerableExtensions.Karnok.impl
{
	...
	internal static class ExceptionHelper
	{
		...
		/// <returns>
		///     True if successful, false if the field already has the terminated
		///     indicator.
		/// </returns>
		internal static bool AddException(ref Exception field, Exception ex)
		{
			for (;;)
			{
				...
				switch (a)
				{
					...
					default:
						if (a is OperationCanceledException && ex is OperationCanceledException)
						{
							// Do throw only one OperationCanceled Exception instead of AggregateException
							b = ex;
						}
						else
						{
